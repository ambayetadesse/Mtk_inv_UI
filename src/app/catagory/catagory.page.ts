...
export class CatagoryPage implements OnInit {
  ...
  getCategory() {
    try {
      this.CategoryService.getAllLookUpCategory().subscribe(async result => {
        if (result.length > 0) {
          this.ListOfCategory = await  result;
  ...
  Save() {
    ...
    if (this.regform.valid) {
      if (!this.CatagoryId) {
        this.CategoryService.create(this.regform.value).subscribe(() =>
            this.getCategory()  
        );
      }
      else {
        ...
        this.CategoryService.updateCategory(dataOfCategory).subscribe(() =>
          this.getCategory()    
        );
      }
      this.presentAlert();
      ...
      buttons: [
        {
          ...
          text: 'OK',
          handler: () => {
            this.CategoryService.removeCategory(item.id).subscribe(res=>{
              console.log(res)
              this.getCategory()
            }, error => {
              console.log(error)
